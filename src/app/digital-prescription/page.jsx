    ...
    <main className="min-h-screen bg-white">
        ...
        <div className="container-1280 mx-auto">
          <div className="flex flex-wrap items-center justify-center gap-4 md:gap-6">
            {["📱 iPad & Tablet Optimised", "🖥️ Works on Desktop", "👨‍⚕️ Solo Doctor Workflow", "📤 Digital Prescription to Patient"].map((pill) => (
              <span key={pill} className="font-['Inter'] font-medium text-[14px] text-[#101828]">{pill}</span>
            ))}
          </div>
        </div>
      </section>
            ...
            {STEPS.map((step, i) => (
              <div key={step.num} className="relative flex flex-col">
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 flex flex-col h-full">
                  ...
                  <h3 className="font-['Inter'] font-bold text-[15px] text-[#101828] mb-2">{step.title}</h3>
      ...
      <section className="w-full bg-[#FAFAFA] py-[60px] md:py-[80px] px-4">
        <div className="container-1280 mx-auto">
          <div className="text-center mb-10">
            ...
            <p className="font-['Inter'] text-[16px] md:text-[18px] leading-[28px] text-[#4A5565] max-w-2xl mx-auto">
        ...
        <div className="container-1280 mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="rounded-2xl p-10" style={{ background: "#003d1a" }}>
              ...
              <p className="font-['Inter'] text-[15px] leading-[26px] text-white/80 mb-6">
              ...
              <ul className="flex flex-col gap-3">
                {DELIVERY_LEFT.map((item) => (
                  <li key={item} className="flex items-start gap-3">
                    ...
                    <span className="font-['Inter'] text-[14px] leading-[22px] text-white/90">{item}</span>
                  </li>
                ))}
              </ul>
            </div>
            <div className="rounded-2xl p-10 bg-white border border-gray-100 shadow-sm">
              ...
              <p className="font-['Inter'] text-[15px] leading-[26px] text-[#4A5565] mb-6">
              ...
              <ul className="flex flex-col gap-3">
                {DELIVERY_RIGHT.map((item) => (
                  <li key={item} className="flex items-start gap-3">
                    ...
                    <span className="font-['Inter'] text-[14px] leading-[22px] text-[#4A5565]">{item}</span>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="container-1280 mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {WHO.map((item) => (
              <div
                key={item.title}
                className="bg-white rounded-2xl border border-gray-100 p-7 text-center hover:shadow-md transition-shadow"
              >
                <div className="text-[40px] mb-4">{item.icon}</div>
                <h3 className="font-['Inter'] font-bold text-[18px] text-[#101828] mb-3">{item.title}</h3>
              ...
              <tbody>
                {TABLE_ROWS.map((row, i) => (
                  <tr key={row.feature} className={i % 2 === 0 ? "bg-white" : "bg-[#fafafa]"}>
                    <td className="font-['Inter'] font-medium text-[14px] text-[#101828] px-6 py-4">{row.feature}</td>
                    <td className="font-['Inter'] text-[14px] text-[#4A5565] px-6 py-4">{row.paper}</td>
                    <td className="font-['Inter'] font-semibold text-[14px] text-[#00A63E] px-6 py-4">{row.digital}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              ...
              <Link
                key={link.href}
                href={link.href}
                className="bg-[#f0faf4] rounded-xl p-5 hover:bg-[#dcfce7] transition-colors flex items-center justify-between group"
              >
                <div>
      ...
      <section style={{ background: "#003d1a" }} className="w-full flex justify-center py-[60px] md:py-[80px] px-4">
        <div className="container-1280 w-full text-center">
          ...
          <p className="font-['Inter'] text-[16px] md:text-[18px] leading-[28px] text-white/80 mb-8 max-w-2xl mx-auto">
          ...
          <button
            onClick={openModal}
            className="bg-white text-[#00A63E] font-['Inter'] font-semibold text-[16px] px-8 py-4 rounded-xl hover:bg-[#f0faf4] transition-colors shadow-lg"
          >
            Book Free Demo →
          </button>
          <div className="flex flex-col sm:flex-row justify-center gap-4 mt-4">
            <a
              ...
              className="font-['Inter'] text-[15px] text-white/80 hover:text-white transition-colors"
            >
              contact@ayuplus.com
            </a>
